...
- **Status:** DEFERRED in S1-008. Resolved by S1-010 (DEC-PRE-S1-010-01..04).
...
#### DEC-PRE-S1-010-01 — Approval cycles for resubmission
> Each submission of a Repair Request has its own approval cycle. `repair_request_approval` gains `approval_cycle_no` (smallint, ≥ 1), and the uniqueness of APR-005 becomes **UNIQUE(repair_request_id, approval_step_no, approval_cycle_no)**. A step decided RETURNED_FOR_CORRECTION stays unchanged as decision history, and routing after a resubmission creates the step in the next cycle.

- **Source:** Portfolio Project Owner, S1-010 pre-implementation review, 16 September 2026.
- **Why:** ST-RR-006 and UAT-02 require "decision history retained", but APR-005 "Unique request/step" allows only one row per step (the dependency named in DEC-PRE-S1-008-02). Reusing the row would overwrite the returned decision and its reason.
- **Rules:**
  - The current cycle of a step is its highest `approval_cycle_no`.
  - Routing: a current RETURNED_FOR_CORRECTION row → the next cycle; an unassigned routing-failure row → routed again within its own cycle (DEC-PRE-S1-007R-09 unchanged); an assigned or other decided row → 409.
  - Approve / Reject / Return decide the current-cycle step only.
...
- **Source:** Portfolio Project Owner, S1-010 pre-implementation review, 16 September 2026.
- **Why:** the same reasoning as DEC-PRE-S1-008-03: a SUBMITTED request never has an assigned approver. The transition matrix is not changed; the command guard is stricter.
- **Also recorded for S1-010 (no new decision):**
  - the reason is required (UC-RR-003; APR-008 "Required Reject/Return"), trimmed, 1..1000 characters, and stored in APR-008 `decision_reason` and as the audit reason. `repair_request` has no return-reason column and none is added;
...
| NB-3 | Resubmit after Return for Correction: Request No stays unchanged (BR-01 immutable). SLA restart/continuation semantics on resubmit are not decided | RESOLVED by DEC-PRE-S1-010-02 (16 September 2026): Request No, submitted_by and submitted_at are kept; the SLA continues |
...
| DEC-PRE-S1-008-02 | UC-RR-003; ST-RR-006; NB-3 | ST-RR-006 (resolved by S1-010) | RR-API-008 (ADD §4.9) | TC-RR-008 | See DEC-PRE-S1-010-01..04 |
| DEC-PRE-S1-010-01..04 (Return / Resubmit) | UC-RR-002/003; BR-01/14; D-12; RR-DD-001 RR-003/011/014, APR-005..010 (APR-005 amended); UAT-02/03 | ST-RR-006 (UNDER_REVIEW only) and ST-RR-002 again; ST-RR-003 into the next cycle | RR-API-008, RR-API-005 resubmission (ADD §2, §4.1, §4.9) | TC-RR-008; TC-RR-004; TC-SEC-001 | RepairRequestReturnForCorrectionDomainTests; RepairRequestReturnForCorrectionServiceTests (incl. CycleOneAssignee_CannotDecide_WhenTheCurrentCycleIsAssignedToAnotherApprover); RepairRequestResubmitServiceTests; RepairRequestRoutingServiceTests (ReturnedApproval_*); RepairRequestReturnResubmitTests (ReturnEditResubmit_RoutesTheNextCycle_KeepsHistoryRequestNoAndSubmitData_AndCanBeApproved, ResubmitWithNoActiveRoute_StaysSubmitted_IsListedForRecovery_AndAdminRetryAssignsTheNextCycle, Resubmit_RerunsTheDuplicateRule_WithoutMatchingItself_AndTheAppliedReasonReplacesTheStoredOne, ResubmitRoutesTheCurrentlyEligibleApprover_AndTheCycleOneApproverCannotDecideCycleTwo, ReturnRacingAnotherCommandOnUnderReview_ExactlyOneWins, ResubmitRacingCancelOrAnotherResubmit_ExactlyOneWins_AndOnlyOneCycleIsRouted, ReturnWhileTheReviewLockIsHeld_IsConcurrencyConflict_AndWritesNothing, GenuineDatabaseFailure_RollsBackTheReturn_AndReleasesTheLock, GenuineDatabaseFailureDuringResubmit_RollsBackEverything, Database_AllowsOneApprovalRowPerRequestStepAndCycle); MigrationSeedTests.UpgradingAnExistingDatabase_PlacesExistingApprovalRowsInTheFirstCycle; RepairRequestReturnForCorrectionEndpointsTests |
...
## 11. Sprint 2 / S2-001 Decisions (Portfolio Project Owner Approval, 17 September 2026)

Pre-implementation decisions for S2-001 "Work Order List", resolving the open items raised by the companion decision sheet `docs/14_Sprint2_S2-001_WorkOrder_List_Decision_Sheet_v1.0.md`. As with every decision in this register, nothing here changes locked business semantics; where a decision requires new persisted content not stated by the baseline, it is marked as such rather than presented as pre-existing.

| Decision | Statement | Rationale / Source |
|---|---|---|
| DEC-S2-001-01 | Work Order List/Detail display `customerCode`, `siteCode`, `equipmentCode`, labeled **"Customer Code"**, **"Site Code"**, **"Equipment Code"** respectively — not display names, since `Customer`, `Site` and `Equipment` each expose only a code field today (`backend/src/RepairRequest.Domain/MasterData/{Customer,Site,Equipment}.cs`). The open question in `DEC-PS1-001` ("is one Customer record synonymous with one Tenant, or can a Tenant contain multiple Customers?") is **not resolved by this decision** and remains OPEN. | `docs/14` §2; `DEC-PS1-001` (Section 1, lines 38-58) |
| DEC-S2-001-02 | Scheduled Date and Technician are **not shown** in the Work Order List or Detail in S2-001. A Work Order has 0..N Service Visits (`SV-003` is a plain, non-unique FK) and no baseline document defines how to select one visit's date/technician to represent the Work Order row. This is DEFERRED to the future Schedule/Service Visit ticket, which must define the selection rule (or an explicit "ambiguous" UI state) before these columns are added. | `docs/14` §3; `docs/03` §4 (ST-SV-001..009); `docs/05` Service Visit table; D-15 |
| DEC-S2-001-03 | The Work Order read scope mirrors `IDataScope.RepairRequests(CurrentUser)` exactly (site-wide roles see Work Orders whose linked Repair Request's Site is in the caller's `user_site_scope`; REQUESTER sees only Work Orders linked to Repair Requests they created), correlated through the Work Order's `repair_request_id`. The new `WorkOrder.Read` authorization policy allows `{REQUESTER, APPROVER, COORDINATOR, TEAM_LEAD, SUPERVISOR}`. **TECHNICIAN is explicitly excluded** until Work Order/Visit assignment rules are defined (Portfolio Project Owner directive, 17 September 2026); ADMINISTRATOR is excluded per the existing no-business-read convention (`DataScope.cs` line ~100). Additionally, **no "Team" master-data entity exists anywhere in the codebase** — only the `TEAM_LEAD` login role (`RoleCodes.cs`) — so `owner_team_id` (WO-006) cannot be resolved to a display value. Team Lead (`team_lead_id`, WO-007) could resolve via the existing `ApplicationUser`/`AspNetUsers` table, but the Portfolio Project Owner chose to **omit both** Assigned Team and Team Lead from the S2-001 UI, matching the treatment of Scheduled Date/Technician. `owner_team_id`/`team_lead_id` are still persisted on the `work_order` table per WO-006/WO-007 — display is deferred, not the schema. | `docs/14` §4; `docs/12` §8 line 877 ("Sprint 2 team / assignment scope... DEFERRED — Sprint 2"); `AuthorizationPolicies.cs` line 8 |
| DEC-S2-001-04 | `GET /api/v1/work-orders` supports a `status` filter (validated against the `WorkOrderStatus` enum) and `page`/`pageSize` paging (existing `Paging:DefaultPageSize`/`MaxPageSize` convention), with a fixed **newest-first** sort — no client-selectable sort parameter in this ticket. Newest-first is implemented via a new **technical** `created_at` column on `work_order`. This field is **not** one of the documented WO-001..013 business fields — it is added purely as ordering support, the same treatment already given to `row_version` (WO-013), because (a) no Sprint 1 entity or shared base class carries any creation-timestamp convention to inherit (confirmed by a full-codebase search), and (b) `work_order_no`'s (WO-003) generation format is documented only as "Unique generated" with no fixed-width/sequential format specified (unlike `request_no`, RR-003, whose `RR-{yyyy}-{000000}` format is fully specified and does support reliable string-sort ordering) — so it cannot be relied on for chronological order. | `docs/14` §5; `docs/09` §3/§6/§7/§9; `docs/13` §1/§3.4; `docs/05` WO-003, WO-013 |
