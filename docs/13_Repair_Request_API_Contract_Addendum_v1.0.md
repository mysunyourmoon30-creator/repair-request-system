...
Companion to RR-API-001 v1.2. It documents what is already implemented, plus contract amendments that are approved but not yet implemented.
...
| Implementation evidence | S1-002 (d2e96c5), S1-003 (7a4be91), S1-004 (c03c4c4), S1-005 (46f12a6), S1-006 (ce69f6e) |
| Approval | Portfolio Project Owner Approval (DEC-PS1-016) |

**Rules for this addendum:**
- The RR-API-001 PDF is not modified.
- An endpoint is listed as implemented only if a controller route exists in the S1-006 codebase.
...
- Section 5 lists approved amendments to catalogued endpoints that are **not yet implemented**; they are marked as such.
...
| ID | Method / Path | Status at S1-006 |
|---|---|---|
| RR-API-001 | POST `/api/v1/repair-requests` | IMPLEMENTED (S1-005) — field subset, see §4.1 |
| RR-API-002 | PATCH `/api/v1/repair-requests/{id}` | IMPLEMENTED (S1-005) — field subset, see §4.1 |
| RR-API-003 | GET `/api/v1/repair-requests` | NOT IMPLEMENTED |
| RR-API-004 | GET `/api/v1/repair-requests/{id}` | IMPLEMENTED (S1-005) |
| RR-API-005 | POST `/api/v1/repair-requests/{id}/submit` | NOT IMPLEMENTED — approved amendments in §5 (S1-007) |
...
- Body (all optional while DRAFT, RR-DD-001 Y@Submit): `siteId`, `equipmentId`, `description`, `preferredStartAt`, `preferredEndAt`.
- Timestamps must be ISO-8601 with an explicit offset; otherwise 400.
- Not accepted at S1-006 (DEC-S1-005-E1, superseded; see §5): `requestCategoryCode`, `priorityCode`, `requestContactId`, `locationId`.
- `201 Created` with `Location` and `ETag`.
- Response: `{ id, status, requestNo, siteId, equipmentId, description, preferredStartAt, preferredEndAt, createdBy, rowVersion }`.
- Errors: Site outside the caller's business Site scope → 404. Inactive Site/Equipment, Equipment not in Site, Equipment without Site, description >2000, end < start → 422.
...
- The body carries the complete S1-005 editable field set, as saved by the Draft form; an omitted field is saved as empty.
- The whole resulting Draft is validated on every save (DEC-S1-005-E2).
...
- Detail within the caller's approved Repair Request scope (DEC-S1-005-E3; S1-003).
...
- No malware scan trigger endpoint; the scan runner / background Worker is DEFERRED.
...
## 5. Approved Contract Amendments — NOT YET IMPLEMENTED (S1-007)

These come from the Pre-S1-007 requirement resolution (RR-DEC-001 §7). They describe the target contract only. **None of them is implemented at S1-006.**

| Endpoint | Amendment | Decision |
|---|---|---|
| RR-API-001 / RR-API-002 | Accept and return `requestCategoryCode` and `priorityCode` (tenant-scoped seeded lookup codes) and `requestContactId` (active user in selected-Site scope). `locationId` remains **not accepted** in Sprint 1 | DEC-PRE-S1-007-01, -02, -04, -05; supersedes DEC-S1-005-E1 |
...
| RR-API-005 | Without at least one CLEAN JPG/JPEG/PNG attachment → 422 VALIDATION_FAILED; request stays DRAFT. PENDING and FAILED attachments do not satisfy this requirement. Additional PENDING attachments do not block Submit. Whether an additional FAILED attachment blocks Submit is **not decided — MUST RESOLVE AT S1-007 START** (RR-DEC-001 NB-5) | DEC-PRE-S1-007-06, -07 |
...
- Read-only lookup endpoints for Category, Priority and eligible contacts are an S1-007 design item (RR-DEC-001 §9). No such endpoint exists today.
...
| Submit (S1-007, not implemented) | FR-02; BR-01/02/14/16; D-12; RR-DD-001 RR-014; DEC-PRE-S1-007-01..12 | ST-RR-002 (SLA start = `submitted_at`); SLA calculation / EV-SLA-001..005 deferred | RR-API-005 (§5) | TC-RR-003/004; TC-SEC-001/002 (TC-SLA-\* deferred) | — (S1-007) |
