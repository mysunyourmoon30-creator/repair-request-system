...
/// <summary>
/// Pause (ST-WS-002; UC-WO-012; S3-002) of the Technician's own Work Session, plus the read of their current
/// session that the Pause control needs. One Pause runs in one transaction, and the checks run in this
...
/// CONCURRENCY_CONFLICT, checked against the session's own RowVersion); state CHECKED_IN (409 STATE_CONFLICT —
/// a second Pause of an already-paused session lands here); a non-blank reason (422). Success sets PAUSED,
/// appends a pause period (never replacing an earlier one) and audits, all with server-derived time. The Work
/// Order and the Visit stay IN_PROGRESS — RR-STS-001 defines no Work Order or Visit transition for Pause.
/// </summary>
public sealed class WorkSessionService
{
    ...
    private async Task<CommandResult<Guid>> PauseLockedAsync(
        CommandContext context, Guid workSessionId, byte[] expectedRowVersion, string? reason, CancellationToken cancellationToken)
    {
        var session = await _store.LoadOwnForPauseAsync(context.User, workSessionId, cancellationToken);
        if (session is null)
        {
